...
class DevToolManagerApp(ctk.CTk):
    ...
    def _refresh_info(self):
        ...
        self._env_values["Python"].configure(text=sys.version.split()[0] if sys.version else "N/A")
        self._env_values["Platform"].configure(text=sys.platform)
        self._env_values["Executable"].configure(text=sys.executable)

        pip_ver = self._get_pip_version()
        self._env_values["Pip"].configure(text=pip_ver)
    ...
    def _get_pip_version():
        try:
            return subprocess.check_output([sys.executable, "-m", "pip", "--version"], text=True).strip()
        except Exception:
            return "N/A"
